...
def _top_rule(rows: list[ReportDetail]) -> str | None:
    ...
    paths are meant to be indistinguishable to a caller, and they were not.

    "none" is also a real rule value meaning no Life-Saving Rule applies, so returning it
    here told the frontend a site's top precursor rule was "none" when the truth was that the
    site had no precursors at all.
    """
