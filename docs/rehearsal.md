...
| **Why we don't quote a ceiling** | Our first round was independent and agreed 52%, kappa 0.083 — severity was the gate that split us, so we rewrote it. The second round agreed 96% but was not run independently, so we cannot separate the rubric's effect from the loss of independence. A ceiling needs a fresh independent pass, and we would rather report that number than one we cannot defend. |
