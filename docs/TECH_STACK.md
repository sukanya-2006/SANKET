...
- **What actually happened — record it, don't round it off.** Round one ran independently and agreed **52.2%** on `is_sif_precursor`, Cohen's kappa **0.083**. Severity was the gate that split us: 14.4% agreement, 3–4 points apart on 76 of 180 reports. The <70% rule fired as written — the severity bands were rewritten from adjectives into observable outcomes and the one-change rule was moved to the front (rubric v2.1 → v2.2), and only severity was re-labelled. Round two scored 96.1% / kappa 0.922, **but round two was not run independently, and both rounds were labelled under v2.1** (confirmed by Member 1, 11 Sep; all 173 gold rows record `rubric_version 2.1`). So the revision was never applied to a label and cannot be credited for the jump — the loss of independence is the only known difference between the rounds, and it is a sufficient explanation on its own. 173 reports are agreed; 7 stay open (10, 20, 51, 60, 96, 139, 149). The adjudicated labels are good gold labels for training and evaluation; and the defensible agreement number is kappa 0.947 on a 38-report independent subset. Getting one is a fresh independent pass over a ~40-report subset, no contact between annotators — a couple of hours' work, and we would rather report that number than one we cannot defend.
...
Report F1 and PR-AUC, never accuracy. Recall over precision, stated as a design choice. Plus: annotator agreement % and Cohen's kappa — always quoted with the round they came from, never as a ceiling. Round one (independent): 52.2% / 0.083. Round two (**not independent — not a ceiling**): 96.1% / 0.922. A ceiling claim needs a fresh independent pass; until we run one, we don't quote one.
...
1. `eval/run_eval.py` → one output: baseline F1, LLM F1, OSHA-set LLM F1, severity MAE. No human-ceiling line — see Evaluation: the independent agreement number is kappa 0.947 on a 38-report subset.
...
---
*Changelog (labelling correction, twice): the round-two figures (96.1% / kappa 0.922) were first carried as an inter-annotator ceiling. Round two was not run independently, so that reading was withdrawn and they were relabelled "evidence the v2.2 revision worked". That second reading is withdrawn too — Member 1 confirmed on 11 Sep that BOTH rounds used v2.1, so the revision never touched a label. The figures stay as a record of what the two rounds scored, credited to nothing. The independent number is round one: 52.2% / 0.083. `run_eval.py` no longer prints a human-ceiling line.*

*Note, unresolved: the shipped prompt encodes rubric v2.2 while the gold labels encode v2.1, so part of every model-versus-gold disagreement is a rubric mismatch rather than model error. It lands hardest on Gate 3. Any re-labelling should be run under v2.2.*
