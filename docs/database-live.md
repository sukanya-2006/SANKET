...
The branch this document was written on sat unmerged for two days while main moved ahead, so
several of the fixes below were re-applied to main rather than merged. Everything here is now
on main, plus a second round of repairs described in the 11 Sep commit:

- The seeded stub had been removed entirely, so a fresh clone served an empty API and seven
  tests failed.
- `requirements.txt` had become UTF-16 from a PowerShell `pip freeze`.
- A missing `python-multipart` was taking the Groq classifier down with it and the log blamed
  `GROQ_API_KEY`.
- 855 lines of commented-out routes were back in `routes.py`.
- The admin dashboard had never displayed a real number - `getSites()` returns
  `{ranked, insufficient_volume, min_group_n}` and the page read a `sites` key that does not
  exist, so it fell through to invented data on every load.
- 19 worker submissions were stored as `source='synthetic'`.

The classifier's failure mode has also moved from Gate 3 to Gate 2. See
[../eval/eval_summary.md](../eval/eval_summary.md) for the current honest numbers.
